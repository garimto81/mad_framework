...
class TextGradOptimizer:
    ...
    def analyze_prompt(self, prompt: str) -> list[TextGradient]:
        ...
        if (
            "예:" not in prompt
            and "예시" not in prompt
            and "example" not in prompt.lower()
        ):
            gradients.append(
                TextGradient(
    ...
    def optimize_prompt(
        self, prompt: str, max_iterations: int = 5, score_threshold: float = 0.9
    ...
    def optimize_agent(
        self, agent_name: str, agent_prompt: str, max_iterations: int = 5
        ...
        Returns:
            최적화 결과
        """
        optimized_prompt, steps = self.optimize_prompt(agent_prompt, max_iterations)
    ...
    def optimize_all_agents(
        self, agents_dir: str, max_iterations: int = 5
        ...
        for agent_file in agents_path.glob("*.md"):
            ...
            result = self.optimize_agent(agent_name, agent_prompt, max_iterations)
    ...
    def generate_report(self, results: list[AgentOptimizationResult]) -> str:
        ...
        avg_improvement = (
            sum(r.total_improvement for r in results) / total_agents
            if total_agents > 0
            else 0
        )
